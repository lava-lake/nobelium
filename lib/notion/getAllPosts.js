...
export async function getAllPosts ({ includePages = false }) {
  ...
  console.log(`getAllPosts: ${data.length} total entries before filter`)
  // Debug: log entries with dates in 2026
  data.filter(p => p?.date?.start_date?.startsWith('2026') || p?.createdTime?.includes('2026'))
    .forEach(p => console.log(`  2026 entry: title="${p.title}", type=${JSON.stringify(p.type)}, status=${JSON.stringify(p.status)}, date=${JSON.stringify(p.date)}`))
  const posts = filterPublishedPosts({ posts: data, includePages })
  console.log(`getAllPosts: ${posts.length} posts after filter`)
