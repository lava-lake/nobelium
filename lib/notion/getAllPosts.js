...
export async function getAllPosts ({ includePages = false }) {
  ...
  if (!pageIds.length) {
    ...
    if (collectionId && collectionViewId) {
      try {
        // Fetch all pages using pagination (Notion may have 1000+ entries)
        let allBlockIds = []
        ...
        do {
          const loaderConfig = {
            type: 'reducer',
            reducers: {
              collection_group_results: { type: 'results', limit: PAGE_SIZE }
            },
            searchQuery: '',
            userTimeZone: 'Asia/Hong_Kong'
          }
          if (cursor) {
            loaderConfig.reducers.collection_group_results.cursor = cursor
          }
          ...
          const result = queryData.result?.reducerResults?.collection_group_results || {}
          const batchIds = result.blockIds || []
          allBlockIds = allBlockIds.concat(batchIds)

          // Check if there are more pages
          cursor = result.hasMore ? { stack: result.cursor?.stack } : undefined
          if (!result.hasMore || batchIds.length < PAGE_SIZE) {
            cursor = undefined
          }
        } while (cursor)
